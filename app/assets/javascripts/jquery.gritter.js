...
jQuery(document).ready(function($) {
	...
	Gritter = {
		...
		add : function(user, text, image, sticky, time_alive) {
			...
			tmp = this.str_replace(['[[username]]', '[[text]]', '[[image]]', '[[number]]', '[[class_name]]'], [user, text, image_str, this._item_count, class_name], tmp);
		...
		remove : function(e) {
			...
			$(e).parents('.gritter-item-wrapper').fadeOut('medium', function() {
				$(this).remove();
			});
		...
		setFadeTimer : function(item, number) {
			...
			Gritter['_int_id_' + number] = window.setTimeout(function() {
				Gritter.fade(item);
			}, timer_str);
	...
	$.gritter.add = function(params) {
		...
		var id = Gritter.add(params.title, params.text, params.image || '', params.sticky || false, params.time || '');
